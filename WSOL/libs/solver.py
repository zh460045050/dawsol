...
from libs.wsol.model.backbone import resnet50, inception_v3, resnet50_adl, vgg16
...
class Trainer(object):
    ...
    def __init__(self):
        ...
        print(self.args)
        if self.args.architecture == 'inception_v3' or self.args.architecture == 'vgg16':
    ...
    def _set_model(self):
        ...
        if self.args.architecture == 'inception_v3':
            ...
            extractor = resnet50_adl(
                ...
                acol_drop_threshold=self.args.acol_threshold,
                num_head = self.args.num_head)
        elif self.args.architecture == 'vgg16':
            extractor = vgg16(
                dataset_name=self.args.dataset_name,
                architecture_type=self.args.architecture_type,
                pretrained=self.args.pretrained,
        ...
        if self.args.architecture == 'vgg16':
            classifier = CAMClassifier(num_classes=num_classes, num_feature=self.feature_dims, init='contant')
        else:
            classifier = CAMClassifier(num_classes=num_classes, num_feature=self.feature_dims)
        ####
        
        if self.args.architecture == 'resnet50' and self.args.dataset_name == "ILSVRC":
            from torch.utils.model_zoo import load_url
            state_dict = load_url('https://download.pytorch.org/models/resnet50-19c8e357.pth', progress=True)
            state_dict['fc.weight'] = state_dict['fc.weight'].unsqueeze(-1).unsqueeze(-1)
            classifier.load_state_dict(state_dict, strict=False)

        #print(classifier.fc.weight.shape, classifier.fc.bias.shape, state_dict['fc.weight'].shape, state_dict['fc.bias'].shape)
    ...
    def _set_optimizer(self):
        ...
        for name, parameter in self.extractor.named_parameters():
            if string_contains_any(
                ...
                if self.args.architecture in ('inception_v3', 'vgg16'):
    ...
    def _wsol_training(self, images, target):
        ...
        if self.args.uda_method != "" and self.epoch > self.args.start_epoch:
            S_T_f, T_t, T_u, batch_label, count, mask = self.TSA.samples_split(image_feature, pixel_feature, target)
        ...
        if self.args.uda_method == 'mmd' and self.epoch > self.args.start_epoch:
            ...
            ###PAMI-Classify the Fake&Target Feature
            loss_T = 0
            loss_T_f = 0
            loss_T_t = 0
            for i in range(0, count):
                cur_S_T_f = S_T_f[i]
                cur_T_t = T_t[i]
                #print(target[i].shape, cur_T_t.shape, cur_S_T_f.shape)
                target_S_T_f = batch_label[i].unsqueeze(0).expand(cur_S_T_f.shape[0]).contiguous().view(-1)
                target_T_t = batch_label[i].unsqueeze(0).expand(cur_T_t.shape[0]).contiguous().view(-1)
                #print(self.classifier(S_T_f[i].view(target_S_T_f.shape[0], -1, 1, 1)).shape)
                logit_S_T_f = self.classifier(cur_S_T_f.view(target_S_T_f.shape[0], -1, 1, 1))
                logit_T_t = self.classifier(cur_T_t.view(target_T_t.shape[0], -1, 1, 1))

                #print(logit_S_T_f.shape, target_S_T_f.shape)
                loss_T_f += self.cross_entropy_loss(logit_S_T_f.view(target_S_T_f.shape[0], -1), target_S_T_f.long())
                loss_T_t += self.cross_entropy_loss(logit_T_t.view(target_T_t.shape[0], -1), target_T_t.long())
            #loss_T_t = self.cross_entropy_loss(self.classifier(T_t).view(images.shape[0], -1), target)
            if count != 0:
                loss_T = loss_T_f / count + loss_T_t / count
            ####
            loss = loss_c + self.args.beta * loss_d + self.args.univer * loss_u + self.args.pami * loss_T

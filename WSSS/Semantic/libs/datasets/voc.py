...
#####
class VOC_WS(_BaseDataset):
    """
    PASCAL VOC Segmentation dataset with WSSS generated label
    """

    def __init__(self, gt_dir="", year=2012, **kwargs):
        self.year = year
        self.gt_dir = gt_dir
        super(VOC_WS, self).__init__(**kwargs)

    def _set_files(self):
        self.root = osp.join(self.root, "VOC{}".format(self.year))

        if self.split in ["train", "train_aug", "trainval", "trainval_aug", "val", "test"]:
            file_list = osp.join(
                self.root, "ImageSets/SegmentationAug", self.split + ".txt"
            )
            file_list = tuple(open(file_list, "r"))
    ...
    def _load_data(self, index):
        ...
        if self.split == "val":
            ...
            label_path = osp.join(self.gt_dir, self.labels[index][1:].split("/")[-1])
        # Load an image
        image = cv2.imread(image_path, cv2.IMREAD_COLOR).astype(np.float32)
        label = np.asarray(Image.open(label_path), dtype=np.int32)
        return image_id, image, label
